...
> **Architecture updates (post-report).** Two production changes landed after this
> report was written; see `PIPELINE_UPDATE_coaching_lifter.md` for the full record:
> 1. **Coaching lifter swapped MotionBERT-Full → GolfPose MixSTE + One-Euro smoothing**
>    (Vicon-verified −63% coaching-measurement error; event detector retrained,
>    PCE@5 holds 0.865). MotionBERT stays a selectable `--lifter` and the PCE
>    leaderboard below is unchanged history.
> 2. **The Unreal Engine mocap handoff is retired.** The player-facing 3D replay is a
>    browser Three.js viewer; BVH export is now opt-in (`pipeline.py --bvh`). Mentions
>    of "UE5 handoff / BVH deliverable" below are historical.
...
Chain: **MediaPipe Lite (2D) → GolfPose MixSTE (3D) → trained 1D-CNN event detector (on raw 3D) + One-Euro smoothing (measurement branch) → 15 biomechanical indicators → coaching scorecard → grounded Claude read + browser 3D replay.**
...
| What does the team get per clip? | Coaching scorecard (PNG + JSON), grounded Claude read, browser 3D replay (Three.js), 3D landmark CSV/JSON, 2D overlay MP4 (BVH mocap opt-in) |
